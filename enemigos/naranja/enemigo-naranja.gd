...
func _ready() -> void:
	...
	if animation_player:
		animation_player.play("enemigo-naranja")

	create_detectors()
	current_velocity.x = speed

	# Detecta jugador automáticamente por nombre
...
func _physics_process(delta: float) -> void:
	...
	# Revisar colisiones con jugadores para empujarlos y atacar
	_check_player_push()
...
func _check_player_push():
	...
	if collider.name == "Caballero" or collider.name == "Robot" or collider.name == "Arquera":
		...
		# Solo hacer daño si todavía puede atacar
		if can_attack:
